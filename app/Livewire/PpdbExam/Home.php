...
class Home extends Component
{
    ...
    {
        $session = PpdbExamSession::where('ppdb_exam_id', $id)->where('ppdb_user_id', Auth::guard('ppdb')->user()->id)->first();
        if ($session) {
            // dd($session);
            return redirect()->route('exam.ppdb.show',  $session->id);
        } else {
            $session = new PpdbExamSession();
            $session->ppdb_exam_id = $id;
            $session->ppdb_user_id = Auth::guard('ppdb')->user()->id;
