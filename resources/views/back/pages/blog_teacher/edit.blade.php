    ...
    <div id="kt_app_content" class="app-content flex-column-fluid">
        <div id="kt_app_content_container" class="app-container container-xxl">
            <form id="kt_ecommerce_add_category_form" class="form d-flex flex-column flex-lg-row"
                action="{{ route('back.blog_teacher.update', $blog_teacher->id) }}" method="POST" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                        ...
                        <div class="card-body text-center pt-0">
                            <style>
                                .image-input-placeholder {
                                    background-image: url('{{ $blog_teacher->getThumbnail() }}');
                                }
                            </style>
                    ...
                    <div class="card card-flush py-4">
                        ...
                        <div class="card-body pt-0">
                            <select name="status" class="form-select mb-2" data-control="select2" data-hide-search="true"
                                ...
                                <option></option>
                                <option value="published" {{ $blog_teacher->status == 'published' ? 'selected' : '' }}>Published</option>
                                <option value="draft" {{ $blog_teacher->status == 'draft' ? 'selected' : '' }}>Draft</option>
                                <option value="archived" {{ $blog_teacher->status == 'archived' ? 'selected' : '' }}>Archived</option>
                            </select>
                            @error('status')
                        ...
                        <div class="card-body pt-0">
                            <div class="mb-10 fv-row">
                                ...
                                <input type="text" name="title" class="form-control mb-2"
                                    placeholder="Judul Berita" value="{{ $blog_teacher->title }}" required />
                                @error('title')
                                    <div class="text-danger fs-7">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input id="keyword_tagify" name="meta_keywords" class="form-control mb-2"
                                    value="{{ $blog_teacher->meta_keywords }}" />
                                @error('meta_keywords')
                                    <div class="text-danger fs-7">{{ $message }}</div>
                                @enderror
                                <div class="text-muted fs-7">
