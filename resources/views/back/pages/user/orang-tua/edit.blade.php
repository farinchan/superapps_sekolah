    ...
    <div id="kt_app_content" class="app-content flex-column-fluid">
        <div id="kt_app_content_container" class="app-container container-xxl">
            <form id="kt_ecommerce_add_category_form" class="form d-flex flex-column flex-lg-row"
                action="{{ route('back.user.parent.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                            ...
                            <div class="mb-5 fv-row">
                                ...
                                <select name="student_id" data-control="select2" data-placeholder="Pilih Siswa" class="form-select mb-2"  required>
                                    ...
                                    @foreach ($list_student as $student)
                                        <option value="{{ $student->id }}" @if ($user->student_id == $student->id) selected @endif>{{ $student->name }}</option>
                                    @endforeach
                                </select>
                                @error('student_id')
                                    <div class="text-danger fs-7">{{ $message }}</div>
